"""SSL/TLS deep certificate and configuration checks."""
...
class SslDeepCheckPlugin(PluginBase):
    name = "ssl_deep_check"
    description = "Deep SSL/TLS certificate and configuration security checks"
    category = "blackbox"
    phase = 1
    destructive_level = 0
    detection_criteria = (
        "Expired/expiring certificate, weak TLS protocol, hostname mismatch, "
        "or certificate verification failure"
    )
    expected_evidence = "Certificate notAfter date, TLS version string, SAN/CN names"
    ...
    async def run(self, target: Target, context: InterPhaseContext | None = None) -> list[Result]:
        if not target.url or not target.url.startswith("https://"):
            return []
        ...
        try:
            ...
            with socket.create_connection((hostname, port), timeout=10) as sock:
                with ctx.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    # 2. Check for weak protocol (TLS 1.0, TLS 1.1)
                    if protocol and protocol in ("TLSv1", "TLSv1.1", "SSLv3"):
                        results.append(Result(
                            ...
                            description=(
                                f"Server negotiated deprecated protocol {protocol}. "
                    ...
                    if all_names and not any(
                        ...
                        results.append(Result(
                            ...
                            cwe_id="CWE-295",
                            recommendation="Use a certificate that matches the server hostname.",
                        ))
