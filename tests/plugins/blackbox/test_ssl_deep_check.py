"""Tests for SSL deep-check plugin (ssl_deep_check)."""
...
from vibee_hacker.plugins.blackbox.ssl_deep_check import SslDeepCheckPlugin
...
def _patch_ssl_connection(cert: dict, protocol: str):
    """Return patch pair that patches socket + ssl for SslDeepCheckPlugin."""
...
class TestSslDeepCheckPlugin:
    @pytest.fixture
    def plugin(self):
        return SslDeepCheckPlugin()
    ...
    # --- run: SSL error is caught and becomes ssl_cert_invalid finding ---

    @pytest.mark.asyncio
    async def test_ssl_error_returns_finding(self, plugin):
        ...
        finding = next(r for r in results if r.rule_id == "ssl_cert_invalid")
        assert finding.base_severity == Severity.HIGH

    # --- run: http target returns empty (not applicable) ---

    @pytest.mark.asyncio
    async def test_not_applicable_returns_empty(self, plugin):
    ...
    # --- run: expired certificate triggers ssl_cert_expired CRITICAL ---

    @pytest.mark.asyncio
    async def test_expired_cert_detected(self, plugin):
        target = Target(url="https://example.com")
    ...
    # --- run: cert expiring soon triggers ssl_cert_expiring MEDIUM ---

    @pytest.mark.asyncio
    async def test_cert_expiring_soon_detected(self, plugin):
        target = Target(url="https://example.com")
        cert, proto = _make_cert(days_left=15, hostname="example.com")
        p1, p2 = _patch_ssl_connection(cert, proto)
        with p1, p2:
            results = await plugin.run(target)
        rule_ids = [r.rule_id for r in results]
    ...
    # --- run: weak TLS protocol triggers ssl_weak_protocol HIGH ---

    @pytest.mark.asyncio
    async def test_weak_protocol_detected(self, plugin):
        target = Target(url="https://example.com")
    ...
    # --- run: valid cert produces no findings ---

    @pytest.mark.asyncio
    async def test_valid_cert_no_findings(self, plugin):
        target = Target(url="https://example.com")
    ...
    # --- run: hostname mismatch triggers ssl_hostname_mismatch HIGH ---

    @pytest.mark.asyncio
    async def test_hostname_mismatch_detected(self, plugin):
        target = Target(url="https://example.com")
        cert, proto = _make_cert(days_left=120, hostname="other.com")
        p1, p2 = _patch_ssl_connection(cert, proto)
        with p1, p2:
            results = await plugin.run(target)
        rule_ids = [r.rule_id for r in results]
        assert "ssl_hostname_mismatch" in rule_ids
        mm = next(r for r in results if r.rule_id == "ssl_hostname_mismatch")
        assert mm.base_severity == Severity.HIGH

    # --- run: connection refused returns empty ---

    @pytest.mark.asyncio
    async def test_connection_refused_returns_empty(self, plugin):
        target = Target(url="https://example.com")
